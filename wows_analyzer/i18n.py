...
TRANSLATIONS = {
    ...
    "ja": {
        ...
        "T-101": (
            ...
            "生存を維持することで {delta_survival_early:+.1f}% の勝率寄与があります。\n\n"
        ...
        "T-102": (
            ...
            "生存に {delta_survival_match:+.1f}% の勝率差があります。\n\n"
        ...
        "T-103": (
            ...
            "それ未満は {wr_short_lived:.1f}% です（差: {delta_time:+.1f}%）。"
        ),
        "T-201": (
            "\nカテゴリ 2 — 自艦ダメージ\n"
            "T-201: 与ダメ絶対値\n"
            ...
            "それ未満は {wr_low_dmg:.1f}% です（差: {delta_damage:+.1f}%）。\n\n"
        ...
        "T-301": (
            ...
            "それ未満は {wr_low_kill:.1f}% です（差: {delta_kill:+.1f}%）。"
        ),
        "T-302": (
            "T-302: ファーストキル\n"
            "あなたが試合で最初のキルを取った場合の勝率は {wr_first_blood:.1f}%、\n"
            "取らなかった場合は {wr_no_first_blood:.1f}% です（差: {delta_first_blood:+.1f}%）。"
        ),
        "T-303": (
            "T-303: K/D 比\n"
            "あなたの平均 K/D は {avg_kd:.2f} です。\n"
            "K/D が {kd_threshold:.1f} を超えた試合の勝率は {wr_high_kd:.1f}%、\n"
            "それ未満は {wr_low_kd:.1f}% です（差: {delta_kd:+.1f}%）。"
        ),
        "T-401": (
            "\nカテゴリ 4 — 駆逐艦専用\n"
            "T-401: 魚雷命中\n"
        ...
        "T-601": (
            ...
            "未満は {wr_weak_enemy:.1f}% です（差: {delta_enemy_wr:+.1f}%）。"
        ),
        "T-602": (
            "T-602: 敵ワースト勝率（弱点敵の影響）\n"
            "敵の最低勝率プレイヤーが {enemy_worst_threshold:.1f}% 未満だった試合の勝率は {wr_enemy_has_potato:.1f}%、\n"
            "そうでない試合は {wr_enemy_no_potato:.1f}% です（差: {delta_enemy_potato:+.1f}%）。\n\n"
        ...
        "T-701": (
            ...
            "劣勢（< 0）だった試合は {wr_sink_trail_t5:.1f}% です（差: {delta_sink_t5:+.1f}%）。\n\n"
            "5 分時点での優位が最終勝敗と一致した割合は {sink_t5_accuracy:.1f}% です。"
        ),
        "T-702": (
            "T-702: 10 分・15 分時点\n"
        ...
        "T-802": (
            "T-802: マップ種別（開け具合）\n"
            "オープンマップ（{open_maps}）での勝率: {wr_open_map:.1f}%\n"
            "クローズドマップ（{closed_maps}）での勝率: {wr_closed_map:.1f}%\n"
            "差: {delta_map_type:+.1f}%\n\n"
        ...
        "T-901": (
            ...
            "トップ→ボトムで {delta_tier_pos:+.1f}% の勝率変化があります。"
        ),
        "T-902": (
            "T-902: ボトムティアでの個人貢献\n"
            ...
            "それ未満は {wr_bot_low_dmg:.1f}% です（差: {delta_bot_dmg:+.1f}%）。"
        ),
        "T-903": (
            "T-903: ティア帯別傾向\n"
